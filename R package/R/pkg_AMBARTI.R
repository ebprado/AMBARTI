...
alessa_ambarti = function(x,
  ...
  n_class_t   = length(classes_t) # number of distinct levels
  aux_comb_t  = 2:floor(n_class_t/2) # levels needed to create the combinations without redundances
  num_comb_t  = choose(n_class_t, aux_comb_t) # total of combinations 2x2, 3x3, up to floor(ne/2)xfloor(ne/2).
  prob_comb_t = num_comb_t/sum(num_comb_t) # probability of observating a combination 2x2, 3x3, up to floor(ne/2)xfloor(ne/2).
  ...
  for (i in 1:TotIter) {
    ...
    for (k in 1:nsteps){
      ...
      for (j in 1:ntrees) {
        ...
        x_e_inter   = create_interaction(x_e, n_class_e, aux_comb_e, prob_comb_e)
        x_g_inter   = create_interaction(x_g, n_class_g, aux_comb_g, prob_comb_g)
        x_s_inter   = create_interaction(x_t, n_class_t, 1, 1)

        x_e_g_inter = cbind(x_e_inter, x_g_inter, x_s_inter)
    ...
    gi = g_e_hat[grepl('^g', colnames(x))]
    ej = g_e_hat[grepl('^e', colnames(x))]
    tk = g_e_hat[grepl('^t', colnames(x))]

    sum_of_squares   = sum((y_scale - y_hat)^2)
    sum_of_squares_g = sum((gi - mu_g)^2)
    sum_of_squares_e = sum((ej - mu_e)^2)
    sum_of_squares_t = sum((tk - mu_e)^2)
    ...
    sigma2_g = update_sigma2_g(sum_of_squares_g, length(gi), a_g, b_g)
    sigma2_e = update_sigma2_e(sum_of_squares_e, length(ej), a_e, b_e)
    sigma2_t = update_sigma2_e(sum_of_squares_t, length(tk), a_e, b_e)
    aux_s2eg = c(rep(1/sigma2_g, length(gi)), rep(1/sigma2_e, length(ej)), rep(1/sigma2_t, length(tk)))
    sigma2_psi_inv = diag(aux_s2eg)
