...
install_github("ebprado/AMBARTI", ref = 'main', auth_token = '0fbb079f1b2c6c830725db8d82218ff58bde9f60') # This my implementation of the semibart idea
library(AMBARTI)
...
alphat_hat_sd = apply(fit.ambarti$beta_hat[,1:10], 2, mean)
plot(1:length(alpha), alpha, col=2, cex=2, main='AMBARTI-Genotype', ylim=c(-8,8), ylab=expression(g[i]), xlab = expression(i)) # true values
points(alpha_hat, cex=2, pch = 2) # estimates
points(alpha_hat - 2*alphat_hat_sd, cex=0.5, pch = 2) # ~95% credible interval: lower bound
points(alpha_hat + 2*alphat_hat_sd, cex=0.5, pch = 2) # ~95% credible interval: upper bound

legend(8,6,'AMBARTI', col=1, pch = 2, bty = 'n')
legend(8,7,'True', col=2, pch = 1, cex=1, bty = 'n')

# Plot the main effects estimates and add the true values
beta_hat = apply(fit.ambarti$beta_hat[,11:20], 2, mean)
beta_hat_sd = apply(fit.ambarti$beta_hat[,11:20], 2, sd)

plot(1:length(beta), beta, col=2, cex=2, main='AMBARTI-Environment', ylim=c(-8,8), ylab=expression(e[j]), xlab = expression(j)) # true values
points(beta_hat, cex=2, pch = 2) # estimates
points(beta_hat - 2*beta_hat_sd, cex=0.5, pch = 2) # estimates
points(beta_hat + 2*beta_hat_sd, cex=0.5, pch = 2) # estimates
legend(8,6,'AMBARTI', col=1, pch = 2, bty = 'n')
legend(8,7,'True', col=2, pch = 1, cex=1, bty = 'n')
...
post_sd = model_run$BUGSoutput$sd

# Plot the main effects estimates and add the true values
alpha_hat = post_means$alpha
alpha_hat_sd = post_sd$alpha

plot(1:length(alpha), alpha, col=2, cex=2, main='AMMI - Genotype', ylim= c(-8,8)) # true values
points(alpha_hat, cex=2, pch = 3, col=4) # estimates
points(alpha_hat - 2*alpha_hat_sd, cex=0.5, pch = 3, col=4) # ~95% credible interval: lower bound
points(alpha_hat + 2*alpha_hat_sd, cex=0.5, pch = 3, col=4) # ~95% credible interval: upper bound

legend(8,5,'AMMI', pch = 3, col=4, bty='n')
legend(8,6,'True', col=2, pch = 1, cex=1)

beta_hat = post_means$beta
beta_hat_sd = post_means$sd
...
points(beta_hat - 2*beta_hat_sd, cex=0.5, pch = 3, col=4) # ~95% credible interval: lower bound
points(beta_hat + 2*beta_hat_sd, cex=0.5, pch = 3, col=4) # ~95% credible interval: upper bound
legend(8,5,'AMMI', pch = 3, col=4, bty='n')
legend(8,6,'True', col=2, pch = 1, cex=1)
